...
function latest_custom1($skin_dir='', $bo_table, $mb_id, $rows=10, $subject_len=40, $cache_time=1, $options='')
{
    global $g5;
    ...
    if( $caches === false ){
        ...
        $tmp_write_table = $g5['write_prefix'] . $bo_table; // 게시판 테이블 전체이름
        $sql = " select * from {$tmp_write_table} where 1 and mb_id='".$mb_id."' and wr_is_comment = 0 order by wr_num limit 0, {$rows} ";
