...
$.widget("ui.plupload", {
	...
	_create: function() {
		...
		if ($.fn.resizable) {
			this.container.resizable({
				handles: 's',
				minHeight: 205 /* 파일함 리사이즈시 최소 사이즈 */
			});
		}
	...
	_initUploader: function() {
		...
		uploader.bind('FileUploaded', function(up, file, result) {
            //var fn = JSON.parse(result.response); // 서버에서 변경된 파일명
            //file['oriname'] = fn.params.filename; 
			self._handleFileStatus(file);
	...
	*/
	...
	_addFormFields: function() {
		...
		plupload.each(this.uploader.files, function(file, count) {
           ...
           // fields += '<input type="hidden" name="' + id + '_oriname" value="'+plupload.xmlEncode(file.oriname)+'" />';
